...
void* display(void *arg) {
    ...
    while (1) {
        ...
        fprintf(Status, "%'-15.0lf%3d%7d%6d%'12.1f%'6.1f%'6.1f%9.4f%7.4f%7.2f%6.2f%'16d    %c", sdr->status.frequency, sdr->status.lna_gain,
                sdr->status.mixer_gain, sdr->status.if_gain, powerdB - (sdr->status.lna_gain + sdr->status.mixer_gain + sdr->status.if_gain), powerdB,
                10 * log10f(sdr->out_power), crealf(sdr->DC), cimagf(sdr->DC), (180 / M_PI) * asin(sdr->sinphi), 10 * log10(sdr->imbalance), sdr->clips, eol);
        fflush(Status);
        usleep(100000); // 10 Hz
    }
    return NULL;
...
double rffc5071_freq(uint16_t lo) {
    ...
    return (REF_FREQ * (tmp_n >> 5ULL) * fbkdiv * FREQ_ONE_MHZ) / (lodiv * (1 << 24ULL));
}
uint32_t max2837_freq(uint32_t freq) {
    uint32_t div_frac;
    //	uint32_t div_int;
