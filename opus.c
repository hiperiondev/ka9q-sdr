...
int main(int argc, char *const argv[]) {
    ...
    while (1) {
        ...
        switch (rtp_hdr.type) {
            ...
            default:
                goto endloop;
                // Discard all but mono and stereo PCM to avoid polluting session table
        }
        ...
        if (sp == NULL) {
            ...
            getnameinfo((struct sockaddr*) &sender, sizeof(sender), sp->addr, sizeof(sp->addr), sp->port, sizeof(sp->port), NI_NOFQDN | NI_DGRAM);
