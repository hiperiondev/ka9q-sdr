...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        while ((pktlen = recv(Input_fd, packet, sizeof(packet), 0)) > 0) {
            ...
            tmp = gmtime(&t);
            if (Logfile) {
                fprintf(Logfile, "%d %s %04d %02d:%02d:%02d UTC ssrc %x seq %d", tmp->tm_mday, Months[tmp->tm_mon], tmp->tm_year + 1900, tmp->tm_hour,
                        tmp->tm_min, tmp->tm_sec, rtp_header.ssrc, rtp_header.seq);
            }
            ...
            if (fprintf(network, "%s\r\n", monstring) <= 0) {
                ...
                network = NULL;
                goto retry;
                // Try to reopen the network connection
            }
        }
        retry:
        ;
