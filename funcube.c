...
int front_end_init(struct sdrstate *sdr, int device, int samprate, int L) {
    ...
    ; // Also the abort target: close handle before returning
...
void* display(void *arg) {
    ...
    while (1) {
        ...
        fprintf(Status, "%'-15.0lf%3d%7d%6d%'12.1f%'6.1f%'6.1f%9.4f%7.4f%7.2f%6.2f%'16d    %8.4lf%'10ld%c", sdr->status.frequency, sdr->status.lna_gain,
                sdr->status.mixer_gain, sdr->status.if_gain, powerdB - (sdr->status.lna_gain + sdr->status.mixer_gain + sdr->status.if_gain), powerdB, powerdB,
                crealf(sdr->DC), cimagf(sdr->DC), (180 / M_PI) * asin(sdr->sinphi), 10 * log10(sdr->imbalance), sdr->overflows, sdr->calibration * 1e6,
                messages, eol);
        messages++;
        fflush(Status);
        usleep(100000);
...
double fcd_actual(unsigned int u32Freq) {
    ...
    struct {
        ...
        UINT32 u32FreqOff;
        UINT32 u32LODiv;
    } *pts, ats[] = { { 4000000U, 130000000U, 16U }, { 8000000U, 130000000U, 16U }, { 16000000U, 130000000U, 16U }, { 32000000U, 130000000U, 16U }, { 75000000U,
            130000000U, 16U }, { 125000000U, 0U, 32U }, { 142000000U, 0U, 16U }, { 148000000U, 0U, 16U }, { 300000000U, 0U, 16U }, { 430000000U, 0U, 4U }, {
            440000000U, 0U, 4U }, { 875000000U, 0U, 4U }, { UINT32_MAX, 0U, 2U }, { 0U, 0U, 0U } };
