...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, optstring)) != EOF) {
        switch (c) {
            ...
            default:
                fprintf(stderr,
                        "Usage: %s [-d doppler_command] [-f frequency] [-I iq multicast address] [-k kaiser_beta] [-l locale] [-L blocksize] [-m mode] [-M FIRlength] [-q] [-R Output multicast address] [-s shift offset] [-t threads] [-u update_ms] [-v]\n",
                        argv[0]);
                exit(1);
                break;
        }
