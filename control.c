...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        ...
        if (memcmp(&old_input_source_address, &demod->input.source_address, sizeof(old_input_source_address)) != 0) {
            ...
            getnameinfo((struct sockaddr*) &demod->input.source_address, sizeof(demod->input.source_address), source, sizeof(source), sport, sizeof(sport),
                    NI_NOFQDN | NI_DGRAM | NI_NUMERICHOST);
        }
        row = 1;
        col = 1;
        wmove(network, 0, 0);
        ...
        mvwprintw(network, row++, col, "Sink: %s; ssrc %8x; TTL %d%s", demod->output.dest_address_text, demod->output.rtp.ssrc, Mcast_ttl,
                Mcast_ttl == 0 ? " (Local host only)" : "");
