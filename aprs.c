...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, "L:M:A:I:vs:")) != EOF) {
        switch (c) {
            ...
            default:
                ...
                fprintf(stdout, "Defaults: %s -L %lf -M %lf -A %lf -s %s -I %s\n", argv[0], latitude, longitude, altitude, Source, Mcast_address_text);
                exit(1);
        }
    }
    ...
    while ((pktlen = recv(Input_fd, packet, sizeof(packet), 0)) > 0) {
        ...
        time(&t);
        tmp = gmtime(&t);
        fprintf(stdout, "%d %s %04d %02d:%02d:%02d UTC", tmp->tm_mday, Months[tmp->tm_mon], tmp->tm_year + 1900, tmp->tm_hour, tmp->tm_min, tmp->tm_sec);
        ...
        if (!isnan(latitude) && !isnan(longitude)) {
            ...
            if (altitude_known)
                fprintf(stdout, " az %.1lf elev %.1lf range %'.1lf m", azimuth * DEGPRA, elevation * DEGPRA, range);
            else
                fprintf(stdout, " az %.1lf range %'.1lf m", azimuth * DEGPRA, range);
        }
        done:
        ;
        fputc('\n', stdout);
