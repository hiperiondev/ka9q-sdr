...
static int pa_callback(const void *inputBuffer, void *outputBuffer, unsigned long framesPerBuffer, const PaStreamCallbackTimeInfo *timeInfo,
...
int main(int argc, char *const argv[]) {
    ...
    while ((c = getopt(argc, argv, "I:vR:B:o:xT:Lf:")) != EOF) {
        switch (c) {
            ...
            default:
                ...
                fprintf(stderr, "Defaults: %s -o %d -B %.1f -I %s -R %s -T %d\n", argv[0], Opus_bitrate, Opus_blocktime, Audiodev, Mcast_output_address_text,
                        Mcast_ttl);
                exit(1);
        }
    }
    // Compute opus parameters
    if (Opus_blocktime != 2.5 && Opus_blocktime != 5 && Opus_blocktime != 10 && Opus_blocktime != 20 && Opus_blocktime != 40 && Opus_blocktime != 60
            && Opus_blocktime != 80 && Opus_blocktime != 100 && Opus_blocktime != 120) {
    ...
    r = Pa_OpenStream(&Pa_Stream, &inputParameters,
    NULL,       // No output stream
            Samprate, Opus_frame_size, // Read one Opus frame at a time
            0, pa_callback,
            NULL);
