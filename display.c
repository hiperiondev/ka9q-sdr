// $Id: display.c,v 1.143 2018/12/05 07:08:01 karn Exp $
...
void* display(void *arg) {
    ...
    for (;;) {
        ...
        mvwprintw(network, row++, col, "Source: %s:%s -> %s:%s; ssrc %0lx", input_source.host, input_source.port, input_dest.host, input_dest.port,
                demod->input.rtp.ssrc);

        mvwprintw(network, row++, col, "IQ pkts %'llu samples %'llu", demod->input.rtp.packets, demod->input.samples);
        ...
        mvwprintw(network, row++, col, "Sink: %s:%s -> %s:%s; ssrc %8x; TTL %d%s", output_source.host, output_source.port, output_dest.host, output_dest.port,
        ...
        switch (c) {
            ...
            case 'k': // Kaiser window beta parameter
            {
                ...
                if (b != demod->filter.kaiser_beta) {
                    ...
                    set_filter(demod->filter.out, demod->filter.low / demod->input.samprate, demod->filter.high / demod->input.samprate,
